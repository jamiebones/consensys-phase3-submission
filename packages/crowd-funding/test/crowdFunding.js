...
let purpose1 = "Helping the poor";
let purpose2 = "Getting rich quick";
...
describe("CrowdFunding", function () {
  ...
  async function setUpContractUtils() {
    ...
    let txn = await crowdFundingFactory
      ...
      .createCrowdFundingContract(fundingId, purpose1, deposit, futureTime, {
        value: deposit,
      });
    
    let wait = await txn.wait();
   
    const cloneAddress = wait.logs[1].args.cloneAddress;
    //load the clone
    ...
    let txnTwo = await crowdFundingFactory
      .connect(otherAccount)
      .createCrowdFundingContract(fundingId, purpose2, deposit, futureTime, {
        value: deposit,
      });

    let waitTwo = await txnTwo.wait();
    const cloneAddressTwo = waitTwo.logs[1].args.cloneAddress;
